...
void alloc_on_move(Alloc& src, Alloc& dst, nestl::true_type)
{
    src = nestl::move(dst);
}

template <typename Alloc>
void alloc_on_move(Alloc& /* src */, Alloc& /* dst */, nestl::false_type) noexcept
{
}

template <typename Alloc>
void alloc_on_move(Alloc& src, Alloc& dst)
{
...
OperationError alloc_on_copy_nothrow(Alloc& src, const Alloc& dst, nestl::true_type) noexcept
{
    return nestl::detail::assign(src, dst);
}

template <typename Alloc, typename OperationError>
OperationError alloc_on_copy_nothrow(Alloc& /* src */, const Alloc& /* dst */, nestl::false_type) noexcept
{
    return OperationError();
}

template <typename Alloc, typename OperationError>
OperationError alloc_on_copy_nothrow(Alloc& src, const Alloc& dst) noexcept
{
    typedef nestl::allocator_traits<Alloc> alloc_traits;
    typedef typename alloc_traits::propagate_on_container_copy_assignment copy_requred;

    return alloc_on_copy_nothrow(src, dst, copy_requred());
}
...
template <typename Alloc>
void alloc_on_copy_throw(Alloc& src, const Alloc& dst, nestl::true_type)
{
    src = dst;
}

template <typename Alloc>
void alloc_on_copy_throw(Alloc& /* src */, const Alloc& /* dst */, nestl::false_type) noexcept
{
    // does nothing
}

template <typename Alloc>
void alloc_on_copy_throw(Alloc& src, const Alloc& dst)
{
    typedef nestl::allocator_traits<Alloc> alloc_traits;
    typedef typename alloc_traits::propagate_on_container_copy_assignment copy_requred;

    alloc_on_copy_throw(src, dst, copy_requred());
}
